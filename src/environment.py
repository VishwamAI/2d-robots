...
class BirdRobotEnvironment(py_environment.PyEnvironment):
    ...
    def _reset(self):
        ...
        for i, obstacle in enumerate(self._obstacles):
            self._state[6 + i * 3 : 8 + i * 3] = (
                obstacle  # Set obstacle positions (obstacle_x, obstacle_y)
            )
            self._state[8 + i * 3] = (
                SENSOR_RANGE  # Initialize obstacle distances to SENSOR_RANGE
            )
        self._episode_ended = False
    ...
    def _step(self, action):
        ...
        if action == self.ACTION_ACCELERATE:
            ...
            self._state[0] += (
                self._state[3]
                * np.cos(np.deg2rad(self._state[2]))
                * SIMULATION_TIME_STEP
            )  # Move forward
            self._state[1] += (
                self._state[3]
                * np.sin(np.deg2rad(self._state[2]))
                * SIMULATION_TIME_STEP
            )  # Move forward
